...
- **Исправленный owner UI contract (26.09.2026):** компактным должен быть блок **🎬 «Сейчас»**, а НЕ левый sidebar. Левое меню возвращено к комфортной базовой ширине **328 px**, нормальным отступам/размерам текста и крупным touch-targets; **«Персонажи / Референсы»** и **«Монтажный разбор»** стоят вертикально друг под другом. Фильтры снова видны напрямую, а **«Служебные файлы»** остаются сворачиваемыми. На desktop/tablet sidebar можно тянуть за правую границу в диапазоне **280–430 px**; ширина хранится локально, двойной клик по границе сбрасывает на 328 px. На mobile ≤820 px resize отключён. Строка authority сокращена до `Drive — канон · GitHub Pages — только чтение`. Плавающая кнопка **↑ Наверх** сохраняется.
...
- Sidebar width is owner-adjustable on desktop/tablet: drag right edge, 280–430 px, persisted locally; double click resets to 328 px. Mobile remains one-column.
- `🎬 Сейчас` оптимизирован как компактная workflow-панель: все 4 группы (`Генерируется сейчас`, `Результат получен`, `Нужно ваше решение`, `Следующие действия`) и их scene links/counts сохраняются, но на широком экране размещаются в компактной 2-column сетке; на узком — одна колонка. Кнопки сцен должны оставаться удобными для пальца.
