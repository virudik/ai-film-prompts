...
class IntegrationRecoveryTests(unittest.TestCase):
    ...
    def test_control_center_separates_scenes_from_generation_slots(self):
        ...
        self.assertIn('id="sideResize"', html)
        self.assertIn('--sidebar-width:328px', html)
        self.assertIn('SIDEBAR_DEFAULT=328', html)
        self.assertNotIn('id="filterPanel"', html)
        self.assertIn('<div id="filters" class="filters"></div>', html)
        self.assertIn('now-card-head', html)
        self.assertIn('.now-card.now-next .now-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))', html)
        self.assertIn("'now-next'", html)
        self.assertIn('Инструкции: СВЕРКА >3 Ч НАЗАД', html)
        self.assertNotIn('ПРОВЕРКА УСТАРЕЛА', html)
        self.assertIn('Drive — канон · GitHub Pages — только чтение', html)
