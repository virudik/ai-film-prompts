...
Важно: Topview уже показывает scenes 2 и 6 как технически завершённые (`success`). Не снимать их автоматически из canonical slow list — сначала проверить каждый результат и решение пользователя.
...
Для canonical slow-сцен таблица `🎬 Активные сцены проекта — карта и навигация` берёт технический статус из `topview-status.json`.
...
Это только отображение технической генерации. Зелёный статус не означает, что ролик принят, и не снимает slow-lock. На текущем checkpoint scenes 2 и 6 завершены технически; 12, 14, 15, 18 ещё ожидаются.
