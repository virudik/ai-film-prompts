...
- `AI Film Recovery Sync` работает **exact hourly ровно в `:00` каждого часа** по Europe/Moscow и является watchdog/fallback. Оба задания стартуют на одной круглой часовой границе; one-writer правило сохраняется: Recovery не должен конкурировать с текущим здоровым циклом watcher и при общей границе часа трактует текущий watcher как выполняющийся/authoritative-in-progress, пока нет доказанного сбоя. Он считает watcher unhealthy если тот disabled, `last_run_time` или telemetry checkpoint старше 75 минут, run не дал checkpoint более 10 минут, либо Board task, существовавший к моменту run, остался вне active+processed state.
