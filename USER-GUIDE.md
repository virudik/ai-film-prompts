...
- На широком экране группы идут компактной сеткой 2×2 с count + scene chips; на меньшем экране — одной колонкой.
- Уменьшать высоту нужно за счёт внутренней компоновки, а не микроскопических шрифтов/кнопок. Scene chips остаются удобными для пальца.
- Левый sidebar не должен снова ужиматься ради этой задачи.
