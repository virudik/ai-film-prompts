...
- **⏳ 4** сцены сейчас в медленной генерации: **20, 21, 22, 23** — это **5 активных Topview-задач / 5 занятых слотов из 6**, потому что Scene 20 запущена дважды. Scenes 17 и 19 технически завершились; Scene 23 вошла в очередь. Повторно не запускать active slow-сцены до результата/ошибки или отдельного решения пользователя.
...
- **Последняя полная синхронизация:** **26.09.2026 · текущая по live Topview/Drive сверке**.
...
| 23 — Люди → коты-джедаи: бесшовное раскрытие второго плана | ⏳ В МЕДЛЕННОЙ ГЕНЕРАЦИИ | Не запускать повторно |
