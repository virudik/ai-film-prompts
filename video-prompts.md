...
- **⏳ 4** сцены сейчас в медленной генерации: **17, 19, 20, 21** — повторно не запускать до результата/ошибки или отдельного решения пользователя.
...
**Синхронизация контекста:** **22.09.2026**. Точное техническое время зеркала хранится в `project-status.json` и здесь не дублируется. Фактическая карта — `film-analysis.md`, статусы и зависимости — `film-backlog.md`. Новые решения пользователя имеют приоритет над старым Notion. В active master остаются 12 актуальных сцен и 22 полных текста промта. Сцены 1, 2, 6, 7, 9, 12, 14, 15 и 18 удалены из active master по прямому решению пользователя как уже отработанные/больше не актуальные; их Scene ID остаются зарезервированы и не переиспользуются. Canonical slow-list сейчас: **17, 19, 20, 21**. Scene 3 получила успешный terminal-result Topview 23.09.2026 20:30:53 UTC и поэтому снята только с render slow-state; Scene 5 также ранее получила успешный terminal-result Topview и снята только с render slow-state; Scene 4 получила terminal `success` 24.09.2026 16:07:36 UTC и также снята только с render slow-state. Ни один технический success не означает автоматическое editorial approval. Scene 20 имеет две активные Topview-задачи (PART 1 и PART 2), остаётся одним Scene ID и занимает два из шести task slots. Scene 21 имеет одну активную Topview-задачу Seedance 2.5; текущая занятость Topview = 5/6. Никакие старые промты Notion сюда автоматически не добавлены. Непрерывный аудиовизуальный контроль не выполнялся.
